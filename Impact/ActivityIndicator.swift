...
//  Impact
//
//  Created by Phillip Ou on 1/26/16.
...
@IBDesignable public class ActivityIndicator: UIView {
    ...
    public init(view:UIView) {
        let centerX = view.frame.size.width/2 - 25
        let centerY = view.frame.size.height/2 - 25
        let frame = CGRect(x: centerX, y: centerY, width: 50, height: 50)
        super.init(frame:frame)
    ...
    private func animate(animated: Bool) {
        if animated {
            ...
            if self.isAnimating == false {
                // create or resume
                if self.circlePathLayer.animationForKey(ActivityIndicatorAnimationKey) == nil {
    ...
    private func createAnimationLayer(layer: CALayer) {
        ...
        layer.addAnimation(animation, forKey: ActivityIndicatorAnimationKey)
    }
